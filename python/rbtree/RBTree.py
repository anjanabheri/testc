...
def testRoutes():
    import random
    print "--- Testing RBList for Routes ---"
    print "    Basic tests..."
    ...
    dup = 0
    while i < 100000:
        routeStr = "%d.%d.%d.%d" %(random.randrange(255), random.randrange(255), random.randrange(255), random.randrange(255))
        ret = rbList.insert (routeStr)
        if (ret != -2):
            i = i + 1
        else:
            dup = dup + 1

    print "Number of Duplicate prefixes generated: %d" %(dup)
